...
const ActivityCalenderTask: React.FC<Props> = (props) => {
    ...
    const getData = () => {
        sRef.current?.getFormData().then((data: any) => {
            ...
            configuration.ntpConfigMgt.getTaskGroup(data.searchDate[0].format(format), data.searchDate[1].format(format), 'activityCalender').then((res:TaskGroup[]) => {
    ...
    return (<>
        <div style={{height: '100%'}}>
            <div>
                ...
                {
                    searchColumns.length && <SearchList cRef={sRef} columns={searchColumns} onFinish={search} isReset={false} />
                }
                <div>
                    <Row>
                        <Col span={6}>
