...
const ConfigDcuArchiveTask: React.FC<Props> = (props) => {
    ...
    const getTaskList = (page: number = 1, groupId: string) => {
        sRef.current?.getFormData().then((data: any) => {
            setLoading(true);
            configuration.ntpConfigMgt.getTaskList(page, ROWS, groupId, data.meter, 'asc', '', 'DESC').then((res:TaskList) => {
                setCurrent(page);
                setTotal(res.total);
    ...
    const getData = () => {
        sRef.current?.getFormData().then((data: any) => {
            ...
            configuration.ntpConfigMgt.getTaskGroup(data.searchDate[0].format(format), data.searchDate[1].format(format), 'specialDay').then((res:TaskGroup[]) => {
