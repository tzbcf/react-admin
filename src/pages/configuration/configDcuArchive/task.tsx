...
let selectGroupId = '';
const ConfigDcuArchiveTask: React.FC<Props> = (props) => {
    ...
    const getTaskType = (record:TaskData) => {
        if (record.FUNC_TYPE === '1') {
            return (<span>{Mes['titleLabelAddadd']}</span>);
        } else if (record.FUNC_TYPE === '2') {
            return (<span>{Mes['btnTitleDelete']}</span>);
        } else if (record.FUNC_TYPE === '3') {
    ...
    const detailColumns = [
        ...
        {
            title: Mes['titleTableTasktypetasktype'],
            dataIndex: 'FUNC_TYPE',
            width: 100,
    ...
    const getTaskList = (page: number = 1, groupId: string) => {
        sRef.current?.getFormData().then((data: any) => {
            setLoading(true);
            configuration.ntpConfigMgt.getTaskList(page, ROWS, groupId, '', 'asc', data.parameters, 'DESC').then((res:TaskList) => {
                setCurrent(page);
                setTotal(res.total);
    ...
    const getData = () => {
        sRef.current?.getFormData().then((data: any) => {
            ...
            configuration.ntpConfigMgt.getTaskGroup(data.searchDate[0].format(format), data.searchDate[1].format(format), 'archive').then((res:TaskGroup[]) => {
...
export default ConfigDcuArchiveTask;
