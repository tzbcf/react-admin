...
const NtpTaskTab: React.FC<Props> = (props) => {
    const { Mes } = props;
    ...
    const getStateIcon = (record:TaskData) => {
        if (record.IS_EXECUTED === '0' || record.IS_EXECUTED === '2') {
            return (<Row><Col span={6}> <LoadingOutlined /></Col><Col span={15}><span></span></Col></Row>);
    ...
    const getTaskList = (page:number = 1, groupId:string) => {
        setLoading(true);
        configuration.ntpConfigMgt.getTaskList(page, ROWS, groupId, '', 'asc', '', 'DESC').then((res:TaskList) => {
            setCurrent(page);
            setTotal(res.total);
    ...
    const getData = () => {
        sRef.current?.getFormData().then((data: any) => {
            ...
            configuration.ntpConfigMgt.getTaskGroup(data.searchDate[0].format(format), data.searchDate[1].format(format), 'deviceNtp').then((res:TaskGroup[]) => {
                setTaskGroupList(res);
                if (res.length) {
                    if (selectedGroupKeys.length) {
                        getTaskList(1, selectedGroupKeys[0]);
                    } else {
