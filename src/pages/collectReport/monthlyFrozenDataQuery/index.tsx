...
const MonthlyFrozenDataQuery: React.FC<Props> = (props) => {
    ...
    const getData = (page: number = 1, rows:number, searchType?:string) => {
        ...
        sRef.current?.getFormData().then((data: any) => {
            ...
            collectReport.frozenData.monthlyFrozenDataQuery(params).then((res:FrozenDataList) => {
                setLoading(false);
                ...
                if (searchType) {
                    ...
                    doc.setFont('font');
                    doc.text('Month Freezen Report', 200, 20);
                    autoTable(doc, {
    ...
    const btnList: BtnConfig[] = [
        {
            ...
            onClick () {
                ...
                //     print('http://localhost:3000/mdr/frozenData/getDayFrozenDataList', params, null);
                // });
    ...
    const searchList = [
        {
            ...
            attr: {
                ...
                picker: 'month',
                style: { width: '100%' },
                defaultValue: [ moment(new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 30), format), moment(new Date(), format) ],
                onChange: changeDate,
                allowClear: false,
            },
        },
    ...
    const initConfig = async () => {
        try {
            ...
            let start = moment(date.setMonth(date.getMonth() - 1)).format(format);
    ...
    return (<>
        <div className='main'>
            <div className='flexCenter flexBetween title'>
                <h4>{Mes['menuTitleMonthlyFrozenDataQuery']}</h4>
            </div>
